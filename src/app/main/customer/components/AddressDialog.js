...
import { addAddress, updateAddress, closeEditAddressDialog, closeNewAddressDialog } from '../store/addressSlice';

function AddressDialog(props) {
  const dispatch = useDispatch();
  const addressDialog = useSelector(({ customersApp }) => customersApp.address.addressDialog);
  ...
  useEffect(() => {
    if (addressDialog.props.open && addressDialog.type === 'edit') {
      ...
      if (addressDialog.data) {
        // console.log('addressDialog.data ', addressDialog.data);
  ...
  function onSubmit(ev) {
    ...
    if (addressDialog.type === 'new') {
      dispatch(addAddress({ ...address, customer }));
    } else {
      ...
      <form onSubmit={onSubmit} className="flex flex-col md:overflow-hidden">
        <DialogContent classes={{ root: 'p-24' }}>
          ...
          <Autocomplete
            ...
            sx={{ width: 300 }}
            renderInput={(params) => (
              <TextField {...params} className="mb-24" label="District" id="district" variant="outlined" required />
            )}
          />
          <Autocomplete
            freeSolo
            ...
            sx={{ width: 300 }}
            renderInput={(params) => (
              <TextField {...params} className="mb-24" label="Province" id="province" variant="outlined" required />
            )}
          />
          <Autocomplete
            freeSolo
            ...
            sx={{ width: 300 }}
            renderInput={(params) => (
              <TextField {...params} className="mb-24" label="zipcode" id="zipcode" variant="outlined" required />
            )}
          />
        </DialogContent>
