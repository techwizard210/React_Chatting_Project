...
function ConfirmEditor(props) {
    ...
    if (response && response.type && response.type === 'carousel') {
      ...
      if (data && data.carousel) {
        ...
        if (data.carousel && data.carousel.columns && data.carousel.columns.length) {
          ...
          if (data.carousel.columns[0] && data.carousel.columns[0].actions && data.carousel.columns[0].actions.length)
  ...
  const onColumnNumberChange = (event) => {
    ...
    for (let index = 0; index < num; index++) {
      if (carouselDataUnsaved.columns[index]) {
        newColumns.push(carouselDataUnsaved.columns[index]);
      } else {
        const newActions = [];
        // eslint-disable-next-line no-plusplus
  ...
  const onActionNumberChange = (event) => {
    ...
    carouselDataUnsaved.columns.forEach((columnData) => {
      ...
      // eslint-disable-next-line no-plusplus
      for (let index = 0; index < num; index++) {
        if (columnData.actions[index]) {
          newActions.push(columnData.actions[index]);
        } else {
          newActions.push({
            type: 'message',
            label: '',
        ...
        <div className="flex flex-col w-full space-y-1 ">
          <div className="flex flex-col space-y-16 items-center">
            ...
            {carouselData.columns &&
              carouselData.columns.map((column, index) => (
