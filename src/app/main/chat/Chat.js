...
import { sendMessage, sendFileMessage } from './store/chatSlice';
import { addSendingMessage } from './store/currentSlice';
import { getReplies } from './store/replySlice';

// import { getReply } from './store/replySlice';
import QuickReplyContent from './components/QuickReplyContent';
import PreviewConfirm from '../settings/reply/reply/components/previews/PreviewConfirm';
import PreviewButtons from '../settings/reply/reply/components/previews/PreviewButtons';
import PreviewCarousel from '../settings/reply/reply/components/previews/PreviewCarousel';
...
function Chat(props) {
  ...
  function shouldShowContactAvatar(item, i) {
    return (
      ...
      ((selected.message[i + 1] && selected.message[i + 1].direction !== 'inbound') || !selected.message[i + 1])
    );
  }
  function isFirstMessageOfGroup(item, i) {
    return i === 0 || (selected.message[i - 1] && selected.message[i - 1].direction !== item.direction);
  }
  ...
  return (
    <div className={clsx('flex flex-col relative', props.className)}>
              ...
              return (
                <StyledMessageRow
                  ...
                  className={clsx(
                    ...
                    i + 1 === selected.message.length && 'pb-96'
                  )}
                >
                  {/* <Hidden mdDown>
                  ...
                  <div className="bubble flex relative items-center justify-center p-12 max-w-xs shadow">
                    ...
                    {item.type && item.type === 'text' && (
                      <p className="break-all whitespace-pre-line">{messageObj.text}</p>
                    )}

                    {item.type && item.type === 'confirm' && (
                      // <div className=" break-all">{JSON.stringify(messageObj.confirm)}</div>
                    ...
                    {/* {item.type &&
                      (item.type === 'buttons' ||
                        item.type === 'confirm' ||
                        item.type === 'carousel' ||
                        item.type === 'flex') && <div className=" break-all">This is a template message</div>} */}
      ...
      {selected && selected.status !== 'none' && (
        <form autoComplete="off" onSubmit={onMessageSubmit} className="absolute bottom-0 right-0 left-0 py-16 px-8">
          <Paper className="flex items-center relative rounded-24 shadow">
            ...
            <Divider className="absolute ltr:left-48 rtl:right-48 top-0 h-32 my-8" orientation="vertical" />
            <IconButton
