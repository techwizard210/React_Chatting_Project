...
import { addCardLabel } from '../store/cardSlice';
...
function CardLabel(props) {
  ...
  useEffect(() => {
    if (labelSelect.length > 10) {
      ...
      dispatch(
        showMessage({
          message: 'Maximum 10 labels per card.',
          variant: 'warning',
        })
      );
    }
  ...
  const insertNewStringLabelList = async (stringLabelList) => {
    dispatch(addCardLabel(stringLabelList)).unwap.then((newCardLabelList) => {
      if (newCardLabelList && newCardLabelList.length) {
        if (newCardLabelList.length + labelSelect.length > 10) {
          dispatch(
            showMessage({
              message: 'Maximum 10 labels per card.', // text or html
          ...
          setValue('labels', [...labelSelect, ...newCardLabelList]);
        }
      }
    });
  };
  ...
  return (
    <Autocomplete
      ...
      renderInput={(params) => <TextField {...params} variant="outlined" placeholder="Todo Label" />}
