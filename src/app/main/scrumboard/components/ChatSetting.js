...
import { updateChat, updateChatOwner } from "../store/boardChatSlice";
...
export function ChatOwnerSetting(props) {
...
export function ChatStatusSetting(props) {
  const classes = useStyles();
  const dispatch = useDispatch();
  const [userList, setUserList] = useState([]);
  const selected = useSelector(({ chatApp }) => chatApp.current.selected);
...
export function TicketDetailSetting() {
  const classes = useStyles();
  const dispatch = useDispatch();
...
export function ChatFollowupSetting(props) {
  const classes = useStyles();
  const dispatch = useDispatch();
  const chat = useSelector(({ chatApp }) => chatApp.current.selected);
  const org = useSelector(({ auth }) => auth.organization.organization);
...
export function OpenHistoryButton(props) {
  const classes = useStyles();
  const dispatch = useDispatch();
  const chat = useSelector(({ chatApp }) => chatApp.current.selected);
