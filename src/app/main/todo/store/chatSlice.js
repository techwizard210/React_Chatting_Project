...
export const getChats = createAsyncThunk('todoApp/chat/getChats', async (chatType, { dispatch, getState }) => {
  try {
...
export const updateChat = createAsyncThunk('todoApp/chats/updateChat', async (chat, { dispatch, getState }) => {
  try {
    const { token } = await firebase.auth().currentUser.getIdTokenResult();
    if (!token) return null;
    const { id: orgId } = getState().auth.organization.organization;
...
export const updateChatOwner = createAsyncThunk('todoApp/chats/updateChat', async (chat, { dispatch, getState }) => {
  try {
    const { token } = await firebase.auth().currentUser.getIdTokenResult();
    if (!token) return null;
    const { id: orgId } = getState().auth.organization.organization;
