...
function TodoDialog(props) {
  ...
  function onSubmit(data) {
    console.log('🚀 ~ file: TodoDialog.js ~ line 110 ~ onSubmit ~ data', { ...data, userId: selectedAccount });
    if (todoDialog.type === 'new') {
      dispatch(addTodo({ todo: { ...data, userId: selectedAccount } }));
    } else {
      dispatch(updateTodo({ todo: { ...todoDialog.data, ...data, userId: selectedAccount } }));
    }
    closeTodoDialog();
  }
    ...
    >
      <>
        ...
        <DialogContent className="p-16 px-0">
          {todoDialog.type !== 'new' && todoDialog.data && todoDialog.data.chatId.length > 0 && selected && (
            <>
                ...
                <div className="flex items-center justify-between p-12">
                  <div className="flex">
                    <Controller
                      ...
                      render={({ field: { onChange, value } }) => (
                        <IconButton tabIndex={-1} disableRipple onClick={(ev) => onChange(!value)} size="large">
                          {value ? (
                  ...
                  <div className="flex items-center justify-start">
                    <Controller
                      ...
                      render={({ field: { onChange, value } }) => (
                        <IconButton onClick={() => onChange(!value)} size="large">
                          {value ? <Icon style={{ color: red[500] }}>error</Icon> : <Icon>error_outline</Icon>}
                        </IconButton>
                      )}
                    />
                    ...
                    <Controller
                      ...
                      render={({ field: { onChange, value } }) => (
                        <IconButton onClick={() => onChange(!value)} size="large">
                          {value ? <Icon style={{ color: amber[500] }}>star</Icon> : <Icon>star_outline</Icon>}
                        </IconButton>
                      )}
                    />
                        ...
                        render={({ field: { onChange, value: formCheckLabels } }) => (
                          ...
                          >
                            {labels.length > 0 &&
                              labels.map((label) => (
                                <MenuItem
                                  onClick={(ev) => onChange(_.xorWith(formCheckLabels, [label], _.isEqual))}
                                  key={label.id}
                                >
                                  <ListItemIcon className="min-w-24">
                                    <Icon color="action">
                                      {formCheckLabels.some((element) => element.title.includes(label.title))
                                        ? 'check_box'
                                        : 'check_box_outline_blank'}
                                    </Icon>
                                  </ListItemIcon>
                                  <ListItemText className="mx-8" primary={label.title} disableTypography />
                                  <ListItemIcon className="min-w-24">
              ...
              <div className="px-16 sm:px-24">
                ...
                <FormControl className="mt-8 mb-16" required fullWidth>
                  <Controller
                    name="notes"
                    control={control}
                    render={({ field }) => <TextField {...field} label="Notes" multiline rows="6" variant="outlined" />}
                  />
                </FormControl>
                ...
                <div className="flex flex-wrap flex-col w-full mb-16">
                  ...
                  <TodoLabel control={control} setValue={setValue} labelSelect={formLabels} labelOption={labels} />
                </div>
                  ...
                  <Controller
                    ...
                    render={({ field: { onChange, value } }) => (
                      <DateTimePicker
                        ...
                        onChange={onChange}
                        minDate={startDate}
                        renderInput={(_props) => <TextField label="Due Date" className="mt-8 mb-16 mx-4" {..._props} />}
                      />
                    )}
                  />
                </div>
